...
export const mirrorNumber = (n: number) => {
  ...
  if (s1 && s2 && s1.length != s2.length) return parseFloat(mirrored.toPrecision(s1.length));
...
export const guessDecade = (number: number) => {
  if (number < 0.5) {
    ...
    if (number <= mirrorNumber(parseFloat('1e' + (approximated + 1)))) return -(approximated + 1);
    return -approximated;
  }
};
...
export const scaleLogit = (): ContinuousScale => {
  ...
  const _ticks = (count = 10) => {
    ...
    if (a <= 0 || a >= 1 || b >= 1 || b <= 0) return [pow10(-1), 0.5, 1 - pow10(-1)];
  ...
  const _tickFormat = function (count?: number, specifier?: any) {
    ...
    if (specifier) return tickFormat(d[0], d[d.length - 1], count == null ? 10 : count, specifier);
  ...
  const scale = scaleCustom(transformLogit, transformLogistic, _ticks, _tickFormat, _nice).domain(logitScaleDefDomain);
